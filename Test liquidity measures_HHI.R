...
model.25 <- lm(HHI ~ I(p_avg^2), data = data)
summary(model.25)
confint(model.25)
pred.25 <- data.frame(HHI = data$HHI,
                      p_avg = data$p_avg,
...
model.26 <- lm(HHI ~ I(vol_tot_100k^2), data = data)
summary(model.26)
confint(model.26)
pred.26 <- data.frame(HHI = data$HHI,
...
ggplot(data = data, aes(x = vol_tot_100k, y = HHI)) + 
  geom_point(size = 0.5) + 
...
model.27 <- lm(HHI ~ I(spread^2), data = data)
summary(model.27)
confint(model.27)
pred.27 <- data.frame(HHI = data$HHI,
                      spread = data$spread,
...
model.28 <- lm(HHI ~ I(trades^2), data = data)
summary(model.28)
confint(model.28)
pred.28 <- data.frame(HHI = data$HHI,
                      trades = data$trades,
...
### SUMMARY HHI measure ### 
results.HHI <- data.frame(model = 1:28,
                            description = c("HHI~p_avg", "HHI~vol_tot", "HHI~spread", "HHI~p_diff", "HHI~trades", "HHI~vol_tot*p_avg", "log(HHI)~p_avg", "log(HHI)~vol_tot", "log(HHI)~spread", "log(HHI)~p_diff", "log(HHI)~trades", "log(HHI)~vol_tot*p_avg", "HHI~log(p_avg)", "HHI~log(vol_tot)", "HHI~log(spread)", "HHI~log(p_diff)", "HHI~log(trades)", "HHI~log(vol_tot)*log(p_avg)", "HHI~sqrt(p_avg)", "HHI~sqrt(vol_tot)", "HHI~sqrt(spread)", "HHI~sqrt(p_diff)", "HHI~sqrt(trades)", "HHI~sqrt(p_avg)*sqrt(vol_tot)", "HHI~(p_avg)^2", "HHI~(vol_tot_100k)^2", "HHI~(spread)^2", "HHI~(trades)^2"),
