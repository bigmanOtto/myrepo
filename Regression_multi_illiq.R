...
## illiq_mid ~ p_avg ## 
model.1 <- lmList(illiq_mid ~ p_avg | cusip_id, data = data)
...
## illiq_mid ~ vol_tot ## 
model.2 <- lmList(illiq_mid ~ vol_tot | cusip_id, data = data)
...
## illiq_mid ~ spread ##   
model.3 <- lmList(illiq_mid ~ spread1 | cusip_id, data = data)
...
model.4 <- lmList(illiq_mid ~ log(vol_tot) | cusip_id, data = data)
...
##illiq_mid~trades##
model.5 <- lmList(illiq_mid ~ trades | cusip_id, data = data)
...
model.6 <- lmList(illiq_mid ~ sqrt(vol_tot) | cusip_id, data = data)
...
model.7 <- lmList(illiq_mid ~ p_avg + vol_tot | cusip_id, data = data)
...
##illiq_mid~p_avg+sqrt(vol_tot)##
model.9 <- lmList(illiq_mid ~ p_avg + sqrt(vol_tot) | cusip_id, data = data)
...
##illiq_mid~sqrt(p_avg)+sqrt(vol_tot)##
model.10 <- lmList(illiq_mid ~ sqrt(p_avg) + sqrt(vol_tot) | cusip_id, data = data)
...
##illiq_mid~p_avg+log(vol_tot)##
model.11 <- lmList(illiq_mid ~ p_avg + log(vol_tot) | cusip_id, data = data)
...
##illiq_mid~log(p_avg)+log(vol_tot)##
model.12 <- lmList(illiq_mid ~ log(p_avg) + log(vol_tot) | cusip_id, data = data)
...
##illiq_mid~vol_tot by rating##
model.13 <- lmList(illiq_mid ~ vol_tot | credit, data = data)
...
##illiq_mid~log(vol_tot) by rating##
model.14 <- lmList(illiq_mid ~ log(vol_tot) | credit, data = data)
...
##illiq_mid~sqrt(vol_tot) by rating##
model.15 <- lmList(illiq_mid ~ sqrt(vol_tot) | credit, data = data)
...
model.19 <- lmList(illiq_mid ~ log(trades) | cusip_id, data = data)
...
model.20 <- lmList(illiq_mid ~ sqrt(trades) | cusip_id, data = data)
...
##illiq_mid~ p_avg + log(vol_tot) + log(trades)##
model.21 <- lmList(illiq_mid ~ p_avg + log(vol_tot) + log(trades) | cusip_id, data = model.21.dataset)
...
##illiq_mid ~ p_avg + log(vol_tot) + trades##
model.22 <- lmList(illiq_mid ~ p_avg + log(vol_tot) + trades | cusip_id, data = model.22.dataset)
...
##illiq_mid ~ p_avg + log(vol_tot) + sqrt(trades)##
model.23 <- lmList(illiq_mid ~ p_avg + log(vol_tot) + sqrt(trades) | cusip_id, data = model.23.dataset)
...
model.24 <- lmList(illiq_mid ~ p_avg + log(vol_tot) + spread1 | cusip_id, data = model.24.dataset)
...
model.25 <- lmList(illiq_mid ~ p_avg + log(vol_tot) + sqrt(spread1) | cusip_id, data = model.25.dataset)
...
##illiq_mid ~ p_avg + log(vol_tot) + spread^2##

model.26 <- lmList(illiq_mid ~ log(vol_tot) + p_avg + (spread1)^2 | cusip_id, data = data)
...
## illiq_mid ~ log(p_avg) ## 
model.27 <- lmList(illiq_mid ~ log(p_avg) | cusip_id, data = data)
...
model.28 <- lmList(illiq_mid ~ sqrt(p_avg) | cusip_id, data = data)
...
model.29 <- lmList(illiq_mid ~ log(spread1) | cusip_id, data = data)
...
model.30 <- lmList(illiq_mid ~ sqrt(spread1) | cusip_id, data = data)
...
model.31 <- lmList(illiq_mid ~ spread1 + vol_tot | cusip_id, data = data)
...
model.32 <- lmList(illiq_mid ~ spread1 + log(vol_tot) | cusip_id, data = data)
...
model.33 <- lmList(illiq_mid ~ spread1 + sqrt(vol_tot) | cusip_id, data = data)
...
## illiq_mid ~ spread + log(vol_tot) + trades## 
model.34 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + trades | cusip_id, data = model.34.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + log(trades)## 
model.35 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + log(trades) | cusip_id, data = model.35.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + sqrt(trades)## 
model.36 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + sqrt(trades) | cusip_id, data = model.36.dataset)
...
## illiq_mid ~ spread + sqrt(vol_tot) + log(trades)## 
model.37 <- lmList(illiq_mid ~ spread1 + sqrt(vol_tot) + log(trades) | cusip_id, data = model.37.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + log(trades) + russell3000_std## 
model.38 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + log(trades) + russell3000_std | cusip_id, data = model.38.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + log(trades) + log(russell3000_std)## 
model.39 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + log(trades) + log(russell3000_std) | cusip_id, data = model.39.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + log(trades) + sqrt(russell3000_std)## 
model.40 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + log(trades) + sqrt(russell3000_std) | cusip_id, data = model.40.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + log(trades) + spbond_std## 
model.41 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + log(trades) + spbond_std | cusip_id, data = model.41.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + log(trades) + log(spbond_std)## 
model.42 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + log(trades) + log(spbond_std) | cusip_id, data = model.42.dataset)
...
## illiq_mid ~ spread + log(vol_tot) + log(trades) + sqrt(spbond_std)## 
model.43 <- lmList(illiq_mid ~ spread1 + log(vol_tot) + log(trades) + sqrt(spbond_std) | cusip_id, data = model.43.dataset)
